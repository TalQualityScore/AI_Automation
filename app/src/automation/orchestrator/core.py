...
class AutomationOrchestrator:
    ...
    def execute_with_ui(self, trello_card_id=None):
        """Execute automation with UI workflow - FIXED to avoid early validation"""
        ...
        try:
            ...
            # FIX: Only fetch card data, don't validate assets yet
            # Step 1: Fetch card data WITHOUT validation
            from ..api_clients import TrelloClient
            trello = TrelloClient()
            self.card_data, error = trello.get_card_data(trello_card_id)
            
            if not self.card_data:
                print(f"❌ Failed to fetch Trello card: {error}")
                return False
            
            # Step 2: Parse instructions WITHOUT asset validation
            from ..workflow_utils import parse_project_info
            
            # Parse basic project info
            project_info = parse_project_info(self.card_data['gdrive_url'])
            if project_info:
                self.project_info = project_info
            else:
                # Create minimal project info
                self.project_info = {
                    'project_name': self.card_data['name'].replace('_', ' '),
                    'ad_type': '',
                    'test_name': '',
                    'version_letter': ''
                }
            
            # Parse processing mode (without validation)
            card_desc = self.card_data.get('desc', '')
            self.processing_mode = self.parser.parse_card_instructions(card_desc)
            
            # NEW: Check for multi-mode instructions (still no validation)
            detected_modes = self.parser.parse_card_instructions_multi(card_desc)
            print(f"🔍 Detected processing modes: {detected_modes}")
            
            # Prepare data for confirmation dialog
            confirmation_data = self.ui_integration.prepare_confirmation_data()
            
            # NEW: Add detected modes to confirmation data if multi-mode
            if len(detected_modes) > 1:
                print(f"🎯 Multi-mode processing detected: {len(detected_modes)} modes")
            ...
            dialog.set_orchestrator(self)  # Set orchestrator reference
            ...
            if success:
                ...
                # Check if it was multi-mode
                if len(detected_modes) > 1:
                    print(f"📁 Created {len(detected_modes)} project folders")
                else:
